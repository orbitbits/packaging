...
key, builds the Jekyll site, generates package directory indexes with Ruby, and
deploys the updated `public/` tree back to GitHub Pages.

Jekyll owns the main site pages and shared CSS. Package directory indexes under
`deb/`, `rpm/`, and `keys/` are generated by `tools/rb/generate_indexes.rb`
because those directories are repository payload and should stay as plain static
files. Repository assembly and package imports also live in `tools/rb/`.
